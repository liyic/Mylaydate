...
//生成年份调整表，用于纵向排列
Dates.adjusts = [0, 9, -1, 8, -2, 7, -3, 6, -4, 5, -5,4,-6,3,-7,2,-8,1,-9, 0];
...
Dates.viewYears = function(YY){
    ...
    Dates.each(new Array(20), function(i){   //i为当前遍历的序号
    
    str += '<li '+ (parseInt(as.year.value) ===(YY+(19-YY%20)-19+i+Dates.adjusts[i]) ? 'class="'+ as[2] +'"' : '') +' y="'+ (YY+(19-YY%20)-19+i+Dates.adjusts[i]) +'">'+ (YY+(19-YY%20)-19+i+Dates.adjusts[i]) +'年</li>';
    //(YY+(19-YY%20)-19+i)         ---横排
	//i%2==0?(YY+(19-YY%20)-19+i)+(YY+(19-YY%20)-19+i)%10:
	//直接使用对应表来处理
    }); 
...
Dates.view = function(elem, options){
	...
	/////////////////////
	   Dates.shde(Dates.query("#laydate_MM"),options.isMonth?true:false); //是否隐藏月
       Dates.shde(Dates.query("#laydate_table"),options.isDay?false:true);  
	   Dates.shde(Dates.query("#laydate_today"),options.isDay?false:true); 
...
Dates.events = function(){
    ...
    Dates.each(S(log.box + ' .laydate_ym'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            ...
            if(!i){
                log.YY = parseInt(as.year.value);     //核心代码在这里
